...
using static Erised.Windows.Interop.User32;
...
internal static partial class Windows
{
    public partial class WindowsEventStream
    {
        ...
        public static Events.Event.KeyEvent? HandleKeyEvent(INPUT_EVENT_RECORD @event, ref ushort? surrogateBuffer)
        {
            var parse = ParseKeyEventRecord(@event);
            if (parse.keyEvent != null)
            {
                return Events.Event.Key(parse.keyEvent.Value);
            }
            ...
            if (parse.surrogate != null)
            {
                ...
                return Events.Event.Key(new KeyEvent(KeyCode.Char(ch.Value), modifiers));
            }
        ...
        public static IEvent? HandleMouseEvent(INPUT_EVENT_RECORD @event, ref MouseButtonPressed pressed)
        {
            ...
            var ypos = (ushort)ParseRelativeY(@event.dwMousePosition.Y);

            var kind = Convert(@event, ref pressed);
            if (kind != null)
            {
                return Events.Event.Mouse(new MouseEvent(kind, xpos, ypos, modifiers));
            }
            ...
            static MouseEventKind.IMouseEventKind? Convert(INPUT_EVENT_RECORD @event, ref MouseButtonPressed pressed)
            {
                ...
                if (eventFlags == EventFlags.MouseMoved)
                {
                    if (isReleased)
                    {
                ...
                if (eventFlags == EventFlags.MouseWheeled)
                {
                    // Vertical scroll
