...
public class PrintStyledContentCommandTest
{
    ...
    public void WriteAnsi_ShouldWriteWithAttribute(Erised.Style.Types.Attribute attribute)
    {
        var text = _fixture.Create<string>();
        ...
        var expected = $"{AnsiCodes.CSI}{attribute.Sgr()}m{text}{AnsiCodes.CSI}{Erised.Style.Types.Attribute.Reset.Sgr()}m";
        writer.ToString().Should().Be(expected);
    }
}